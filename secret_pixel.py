...
def ai_adaptive_lsb_analysis(pixels):

    h, w = pixels.shape[:2]
    flat_size = h * w
    # Use red channel for variance (matches embedding channel)
    r_channel = pixels[:, :, 0].astype(np.float32)
    # Local variance with 3x3 window - textured regions have higher variance
    mean = np.zeros_like(r_channel)
    for dy in range(-1, 2):
        for dx in range(-1, 2):
            shifted = np.roll(np.roll(r_channel, dy, 0), dx, 1)
            mean += shifted
    mean /= 9
    variance = np.zeros_like(r_channel)
    for dy in range(-1, 2):
        for dx in range(-1, 2):
            shifted = np.roll(np.roll(r_channel, dy, 0), dx, 1)
    ...
    # Sort indices by variance descending - most suitable (textured) pixels first
    pixel_indices = np.argsort(-variance_flat)[:flat_size]
    suitability = min(100, int(np.percentile(variance_flat, 75) / 2))
    return suitability, pixel_indices.tolist()
...
def hide_file_in_png(image_path, file_to_hide, output_image_path, public_key_path):
    ...
    # AI-Assisted Adaptive LSB: Use texture analysis to pick least-detectable pixels
    suitability, pixel_indices = ai_adaptive_lsb_analysis(pixels)
    print(f"AI LSB Analysis: Embedding suitability {suitability}% (adaptive pixel selection active)")
...
def extract_file_from_png(image_path, output_file_path, private_key_path):
    ...
    # AI-Assisted Adaptive LSB: Use same texture-based order as encoder
    _, pixel_indices = ai_adaptive_lsb_analysis(pixels)
